...
def _get_features(schema):
  return [
      _column_schema_to_dict_dense(name, column_schema)
...
def _get_sparse_features(schema):
  return [
      _column_schema_to_dict_sparse(name, column_schema)
      for name, column_schema in sorted(six.iteritems(schema.column_schemas))
      if isinstance(
          column_schema.representation,
          dataset_schema.SparseColumnRepresentation,
...
def _column_schema_to_dict_dense(name, column_schema):
  ...
  if column_schema.is_fixed_size():
    axes = [{'size': str(axis.size)} for axis in column_schema.axes]
...
def _column_schema_to_dict_sparse(name, column_schema):
  ...
  return {
      ...
      'valueFeature': [{
          'name':
          representation.value_field_name,
          'type':
          _to_feature_type_enum(column_schema.domain.dtype),
          'domain':
          _to_domain(column_schema.domain),
      }],
  }
...
def _to_feature_type_enum(dtype):
  ...
  return _FEATURE_TYPE_INT if dtype == tf.bool else 'TYPE_UNKNOWN'
...
def _get_tf_options(representation, type_string):
  ...
  if isinstance(representation, dataset_schema.FixedColumnRepresentation):
    if representation.default_value is None:
      ...
      raise ValueError(
          f"v1 Schema can't represent default value {representation.default_value} for type {type_string}"
      )
  ...
  raise TypeError(
      f'Cannot represent {representation} using the Feature representation; the SparseFeature representation should have been chosen.'
  )
